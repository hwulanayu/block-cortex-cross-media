#########################################################{
# PURPOSE
# Provides a template for dynamically creating and formatting HTML links
# for navigation between dashboards, including transfer of filter values.
#
# SOURCE
# none (This template can only be EXTENDED into another view)
#
# STYLES
# This template provides three styles for the html links. Pick which one
# to use with parameter_navigation_style:
#   1. buttons
...
#
# To add or modify styles, edit the link_style_dashboard_navigation constant
# in the Manifest file.
#
# FILTERS
# This template allows a dashboard to pass up to 20 possible filters. The
# dimensions are named filter1 to filter20 and hidden by default.
# Modify this template to add more filter dimensions as needed.
#
#
# STEPS TO EXTEND THIS TEMPLATE {
# When extending this template, make the REQUIRED customizations by
# following these steps:
# 1. Create new view
# 2. Add "extend: template_dashboard_navigation_ext" property
# 3. Edit dimension map_filter_numbers_to_dashboard_filter_names.
#     - Update the "sql" property with the mapping of filter numbers
#       to dashboard filter names.
#     - For each filter that must be passed between dashboards, assign a
#       generic filter number 1 to N.
#     - This can be a broad list of filters that do not need to appear on
#       every dashboard but should appear on at least 2 dashboards.
#     - Filters unique to a single dashboard do not need to be included
#       here (as values do not pass to other dashboards).
#     - Use | between number and dashboard filter name
#     - Use || between each filter
#     - Note if dashboard filter name contains a space use + to represent
#       the space (e.g. Order Date should be entered as Order+Date)
#     - You will specify the specific set of filters used in a dashboard
#       in the dimension dash_bindings
#     - An example for 3 dashboard filters:
#           sql: "1|date||2|business_unit||3|customer_type" ;;
#
# 4. Edit dash_bindings dimension
#    - Update the "sql" property with ID, Link Text and Filter Set
#       of each dashboard:
#
#       ID - For UDD dashboards use numeric id or slug.
#            For a LookML dashboard, use dashboard name with or without model name.
#                 131
#                 model_name::dashboard_name1
#                 dashboard_name1
#
#       Link Text (e.g., Overview)
#
#       Filter Set used on the given dashboard.
#            - Use numbers 1 to N as specified separated by
#              comma (e.g., 1,2 or 1,3,4).
#            - Use numbers as specified in dimension
#              map_filter_numbers_to_dashboard_filter_names
#
...
#
# 5. Update parameter_tab_focus with any extra values needed
#    to match the number of dashboards.
#    Note, when extending, allowed values are additive, so only
#    add values beyond 2.
#
# 6. Edit filter1 to filterN (up to 20) dimensions to set the type,
#    hidden and/or label properties.
#    - Make sure the filter dimension's type matches the field it is
#      mapped to. For example, if filter1 is linked to a date field,
#      change the type to date. The filter dimensions are string
#      by default.
#    - Note, if only using navigation in LookML dashboards, the filter
#      dimensions can remain hidden.
#}
#
...
#           join: cross_media_dashboard_navigation_ext {
